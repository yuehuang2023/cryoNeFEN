...
class PoseTracker(nn.Module):
    ...
    @classmethod
    def load(cls, infile, Nimg, D, emb_type=None, ind=None, device=None, sigma=0):
        """
        ...
        if len(infile) == 2:  # rotation pickle, translation pickle
            ...
            if infile[0].endswith('.txt'):
                ...
                poses=([],[])
                for x in lines:
                    tmp=utils.load_pkl(x.strip())
                    if type(tmp) != tuple:
                        poses[0].append(tmp)
                    else:
                        poses[0].append(tmp[0])
                        poses[1].append(tmp[1])
                if poses[1]:
                    poses=(np.concatenate(poses[0]),np.concatenate(poses[1]))
                else:
        ...
        rots = poses[0]
        if sigma>0:
            rots_noise = lie_tools.expmap(torch.randn([rots.shape[0],3])*sigma).numpy()
            rots = rots @ rots_noise
    ...
    def get_pose(self, ind, device=None):
        if self.emb_type is None:
            rot = self.rots[ind]
        ...
        return rot.to(device), tran.to(device)
