...
def add_args(parser):
    ...
    parser.add_argument(
        "--symmetry", type=str, default="C1", help="Symmetry for training"
    )
...
def train_batch(
    ...
    with torch.no_grad():
        if symmetry is not None:
            symmetry_rot = torch.tensor(symmetry[np.random.choice(symmetry.shape[0],B)], device=lattice.device)
            coords = (lattice.coords.repeat(B,1,1) / lattice.extent / 2  + F.pad(trans,[0,1]).unsqueeze(1) / D) @ rot @ symmetry_rot
        else:
...
def train(data, ctf_params, posetracker, mask, device, ckpt_file=None, outdir=None, args=None):
    ...
    if args.symmetry == "C1":
        ...
        symmetry = generate_symmetry(args.symmetry)
    flog(f"Training with {args.symmetry} symmetry")
